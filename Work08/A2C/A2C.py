...
# 📌 [แก้ตรงนี้: เปลี่ยนการอิมพอร์ตจาก PPO เป็น A2C]
...
def train_model():
    ...
    # 📌 [แก้ตรงนี้: เปลี่ยน agent เป็น A2C และนำพารามิเตอร์ batch_size ออก]
    agent = A2C("MlpPolicy", env_train, learning_rate=0.00025, n_steps=2048, device=device, verbose=1)

    print("\n--- 🚀 [3/3] บอทเริ่มกระโจนเข้าสู่การเรียนรู้แบบคู่ขนาน ---")
    try:
        agent.learn(total_timesteps=150_000, callback=eval_callback) 
        # 📌 [แก้ตรงนี้: เปลี่ยนชื่อไฟล์ที่บันทึกให้สอดคล้องกับ A2C]
        agent.save("a2c_realistic_trading_bot_last")
...
def test_model():
    ...
    # 📌 [แก้ตรงนี้: ปรับแก้ชื่อไฟล์ที่จะทำการโหลดให้เป็นเวอร์ชันของ A2C]
    model_path = "./best_model/best_model.zip"
    if not os.path.exists(model_path):
        model_path = "a2c_realistic_trading_bot_last"

    # 📌 [แก้ตรงนี้: ปรับแก้เป็น A2C.load เพื่อให้โหลดโมเดลกลับมาถูกคลาส]
    trained_model = A2C.load(model_path, device=device)
